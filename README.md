# Hi there, we are Horbit! 👋🇵🇹

<div align="center">
  <img src="https://www.horbit.pt/assets/logo-CpY2WY_o.png" width="150" alt="Horbit Logo"/>
  <br/>
  
  **Premium Hosting Solutions from Portugal.**
  <br/>
  VPS • Web Hosting • Game Servers • Custom Infrastructure
  <br/>
  <br/>
  
  [![Website](https://img.shields.io/badge/Website-horbit.pt-blue?style=for-the-badge&logo=google-chrome)](https://horbit.pt)
  [![Discord](https://img.shields.io/badge/Discord-Join%20Community-5865F2?style=for-the-badge&logo=discord)](https://discord.gg/SEU_LINK_AQUI)
  [![Status](https://img.shields.io/badge/Status-All%20Systems%20Operational-success?style=for-the-badge)](https://status.horbit.pt)
</div>

---

### 🚀 About Us
We are a specialized hosting provider focused on delivering high-performance infrastructure with a personal touch. Based in Portugal, we empower developers and gamers with reliable servers.

### 🛠️ Tech Stack & Infrastructure
* **Virtualization:** Proxmox VE (LXC & KVM)
* **Backend:** Python (Automation), Node.js
* **Frontend:** React, Vite
* **DevOps:** Docker, Git

### 🤝 Support
Need help with your VPS or Game Server?
* 🎫 Open a ticket on our [Client Area](https://horbit.pt/cliente)
* 💬 Join our [Discord Community](https://discord.gg/SEU_LINK_AQUI)

---
<div align="center">
  <sub>© 2026 Horbit. All rights reserved.</sub>
</div>
